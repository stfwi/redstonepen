...
public class Guis
{
  ...
  @OnlyIn(Dist.CLIENT)
  public static abstract class ContainerGui<T extends Container> extends ContainerScreen<T>
  {
    ...
    public ContainerGui(T menu, PlayerInventory player_inv, ITextComponent title, String background_image, int width, int height)
    {
    ...
    public ContainerGui(T menu, PlayerInventory player_inv, ITextComponent title, String background_image)
    {
      super(menu, player_inv, title);
      this.background_image_ = new ResourceLocation(Auxiliaries.modid(), background_image);
      this.player_ = player_inv.player;
    ...
    protected final void renderBg(MatrixStack mx, float partialTicks, int mouseX, int mouseY)
    {
    ...
    protected void renderBgWidgets(MatrixStack mx, float partialTicks, int mouseX, int mouseY)
    {}
    ...
    protected void renderItemTemplate(MatrixStack mx, ItemStack stack, int x, int y)
    {
      ...
      getMinecraft().getTextureManager().bind(background_image_);
  ...
  public static class UiWidget extends net.minecraft.client.gui.widget.Widget
  {
    ...
    public final UiWidget tooltip(Function<UiWidget,ITextComponent> tip)
    { tooltip_ = tip; return this; }

    public final UiWidget tooltip(ITextComponent tip)
    ...
    {
      final MainWindow win = mc_.getWindow();
      return Coord2d.of(
        MathHelper.clamp(((int)(mc_.mouseHandler.xpos() * (double)win.getGuiScaledWidth() / (double)win.getScreenWidth()))-this.x, -1, this.width+1),
        MathHelper.clamp(((int)(mc_.mouseHandler.ypos() * (double)win.getGuiScaledHeight() / (double)win.getScreenHeight()))-this.y, -1, this.height+1)
      );
    }
    ...
    public void renderToolTip(MatrixStack mx, int mouseX, int mouseY)
    {
      if(!visible || (!active) || (tooltip_ == NO_TOOLTIP)) return;
      final ITextComponent tip = tooltip_.apply(this);
  ...
  public static class BackgroundImage extends UiWidget
  {
    ...
    {
      if(!visible) return;
      Minecraft.getInstance().getTextureManager().bind(atlas_);
  ...
  @OnlyIn(Dist.CLIENT)
  public static class TextBox extends net.minecraft.client.gui.widget.TextFieldWidget
  {
    public TextBox(int x, int y, int width, int height, ITextComponent title, FontRenderer font) { super(font, x, y, width, height, title); setBordered(false); }
