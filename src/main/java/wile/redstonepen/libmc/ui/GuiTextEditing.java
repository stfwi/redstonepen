...
public class GuiTextEditing
{
  ...
  public static class MultiLineTextBox extends Guis.UiWidget implements IGuiEventListener
  {
    ...
    public Coord2d getPositionAtIndex(int textIndex)
    {
      ...
      textIndex = MathHelper.clamp(textIndex + lindex - getDisplayCache().lineStarts[lindex], 0, li.contents.length());
    ...
    private void renderCursor(MatrixStack mxs, Coord2d pos, boolean at_end)
    {
      ...
      if(!at_end) {
        AbstractGui.fill(mxs, pos.x, pos.y - 1, pos.x + 1, pos.y + 9, cursor_color_);
      } else {
