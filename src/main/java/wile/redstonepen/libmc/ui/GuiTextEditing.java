...
public class GuiTextEditing
{
  ...
  public static class MultiLineTextBox extends Guis.UiWidget implements IGuiEventListener
  {
    ...
    public MultiLineTextBox(int x, int y, int width, int height, ITextComponent title)
    {
      super(x, y, width, height, title);
      edit_ = new TextInputUtil(
    ...
    public MultiLineTextBox setValue(String text)
    {
      ...
      if(!text_.equals(text)) {
        if(edit_ == null) {
          ...
          edit_.setSelectionRange(cur, sel);
          edit_.setCursorPos(cur, false);
        }
        clearDisplayCache();
      }
      return this;
    ...
    {
      max_text_size_ = MathHelper.clamp(size, 2, 1024);
    ...
    public MultiLineTextBox setFont(FontRenderer fnt)
    ...
    {
      line_height_ = MathHelper.clamp(h, 6, NORM_LINE_HEIGHT);
    ...
    public Coord2d getCoordinatesAtIndex(int textIndex)
    {
      ...
      textIndex = MathHelper.clamp(textIndex, 0, getDisplayCache().fullText.length());
      ...
      textIndex = MathHelper.clamp(textIndex, 0, li.contents.length());
      ...
      if(index >= 0) {
        if((index==last_index_) && ((Util.getMillis()-last_clicked_)<250)) {
          if(edit_.isSelecting()) {
            ...
            // Select word
            edit_.setSelectionRange(CharacterManager.getWordPosition(getText(), -1, index, false), CharacterManager.getWordPosition(getText(), 1, index, false));
          }
        } else {
    ...
    public void renderButton(MatrixStack mxs, int mouseX, int mouseY, float partialTicks)
    {
    ...
    { if(Minecraft.getInstance()!=null) TextInputUtil.setClipboardContents(Minecraft.getInstance(), eotTrimmed(text)); }

    private String getClipboard()
    { return (Minecraft.getInstance()!=null) ? eotTrimmed(TextInputUtil.getClipboardContents(Minecraft.getInstance())) : (""); }
    ...
    private boolean specialKeyMatched(int key)
    {
      ...
      switch(key) {
        case 257: case 335: { edit_.insertText("\n"); return true; }
    ...
    private void renderCursor(MatrixStack mxs, Coord2d pos, boolean at_end)
    {
      if(!active || !visible) frame_tick_ = 0;
      if((++frame_tick_ & 0x3f) < 0x20) return;
      pos = screenCoordinates(pos, true);
      if(!at_end) {
        AbstractGui.fill(mxs, pos.x, pos.y - 1, pos.x + 1, pos.y + NORM_LINE_HEIGHT, cursor_color_);
      } else {
    ...
    private void renderHighlight(Rectangle2d[] line_rects)
    {
    ...
    private DisplayCache rebuildDisplayCache()
    {
      ...
      final List<Integer> lsp = new ArrayList<>();
      ...
      final CharacterManager ssp = font_.getSplitter();
      ...
      final int[] line_starts = lsp.stream().mapToInt(x->(int)x).toArray();
      ...
      List<Rectangle2d> selection_blocks = Lists.newArrayList();
      if(cur_pos != sel_pos) {
      ...
      return new DisplayCache(full_text, ppos, cur_at_eos, line_starts, line_infos.toArray(new LineInfo[0]), selection_blocks.toArray(new Rectangle2d[0]));
    }
    ...
    private Rectangle2d createPartialLineSelection(String text, CharacterManager ssp, int spos, int epos, int liney, int line_start_pos)
    {
    ...
    private Rectangle2d createSelection(Coord2d pos1, Coord2d pos2)
    {
    ...
    {
      static final DisplayCache EMPTY = new DisplayCache("", new Coord2d(0, 0), true, new int[]{0}, new LineInfo[]{new LineInfo(Style.EMPTY, "", 0, 0)}, new Rectangle2d[0]);
      ...
      public DisplayCache(String text, Coord2d cur, boolean at_end, int[] line_starts, LineInfo[] line_data, Rectangle2d[] sel)
      { fullText = text; cursor = cur; cursorAtEnd = at_end; lineStarts = line_starts; lines = line_data; selection = sel; }

      public int getIndexAtPosition(FontRenderer font, Coord2d pos)
      {
    ...
    static class LineInfo
    {
      ...
      { style = fs; contents = s; x = x0; y = y0; asComponent = (new StringTextComponent(contents)).setStyle(style); }
    }
